...
def _job_command(request: JobRequest) -> list[str]:
    ...
    if kind == "analyze":
        ...
        _flag(command, "--effect-density", o.get("effect_density", 0.65))
        _flag(command, "--temporal-persistence", o.get("temporal_persistence", 0.75))
        _flag(command, "--hero-frequency", o.get("hero_frequency", 0.90))
        _flag(command, "--composition-diversity", o.get("composition_diversity", 0.70))
    ...
    if kind == "render":
        ...
        _flag(command, "--native-gpu", o.get("native_gpu", "auto"))
        _flag(command, "--native-hwdecode", o.get("native_hwdecode", "auto"))
        _flag(command, "--native-build-if-missing", o.get("native_build_if_missing", True), boolean=True)
        _flag(command, "--browser-transport", o.get("browser_transport", "auto"))
        _flag(command, "--browser-gpu", o.get("browser_gpu", "auto"))
        _flag(command, "--browser-source-decode", o.get("browser_source_decode", "auto"))
        _flag(command, "--webcodecs-bitrate", o.get("webcodecs_bitrate", 0))
