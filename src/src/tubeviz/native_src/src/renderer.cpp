...
Renderer::Renderer(Manifest manifest, int width, int height, double fps)
    : manifest_(std::move(manifest)), width_(width), height_(height), fps_(fps) {
...
Decoder& Renderer::decoder_for(const std::string& path) {
    ...
    if (it == decoders_.end()) {
        auto decoder = std::make_unique<Decoder>(path, width_, height_);
        it = decoders_.emplace(path, std::move(decoder)).first;
    }
    return *it->second;
}

void Renderer::prune_decoders(const Shot& shot) {
    std::set<std::string> active{shot.primary.path};
    for (const auto& layer : shot.companions) active.insert(layer.path);
    for (auto it = decoders_.begin(); it != decoders_.end();) {
        if (!active.contains(it->first)) it = decoders_.erase(it);
        else ++it;
    }
}
