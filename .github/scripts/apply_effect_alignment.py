...
# Native resident GPU: use the same beat topology and radial geometry as the
# WebGPU/fallback libplacebo renderers, and apply spatial post effects once.
resident = "src/tubeviz/native_src/src/resident_gpu.cpp"
replace_once(
    resident,
...
# Native CPU fallback: same radial ripple/tangential vortex grammar.
effects = "src/tubeviz/native_src/src/effects.cpp"
replace_once(
    effects,
    '''                if (vortex > .015) {
                    px = u - cx; py = v - cy;
...
# Canvas2D fallback: source-over concentric displacement/tangential rotation.
...
replace_once(
  ...
  snapshot();
  const slices=24,sh=height/slices;
  fx.save();fx.globalAlpha=.08+.30*amount;
  for(let i=0;i<slices;i++){
    const y=i*sh,phase0=(i/slices)*Math.PI*4+phase*5;
    const dx=Math.sin(phase0)*width*.018*amount;
    fx.drawImage(scratch,0,y,width,sh+1,dx,y,width,sh+1);
  }
  fx.restore();
}''',
    '''function applyRipple(amount){
...
# Persistent transform defaults: heavy accents are scheduled sparsely and source
# motion no longer receives an always-on ripple residue.
...
# Strong bass accents should not retrigger a full pressure wave on every quarter
# note at common dance tempos.
...
# Regression coverage for the active resident path and both fallbacks.
...
def test_canvas_fallback_uses_radial_source_over_geometry():
    ...
    ripple = source.split("function applyRipple(amount){", 1)[1].split("function applyKaleidoscope", 1)[0]
